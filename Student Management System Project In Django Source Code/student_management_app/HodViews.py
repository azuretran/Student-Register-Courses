...
from .models import CustomUser, Staffs, Courses, Subjects, Students, SessionYearModel, FeedBackStudent, FeedBackStaffs, LeaveReportStudent, LeaveReportStaff, Attendance, AttendanceReport,Staffs_GV_HP,FeedBackStaffs_GV
from .forms import AddStudentForm, EditStudentForm,AddTeacherForm,EditTeacherForm,AddTeacher_GVForm,EditTeacher_GVForm
key = token_bytes(32)
#aes 256 encrypt
...
def add_staff_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            cmnd = form.cleaned_data['cmnd']
            birthday = form.cleaned_data['birthday']
            salary = form.cleaned_data['salary']
            ciphertext=encrypt(salary)
...
def manage_staff(request):
    cipher = AES.new(key, AES.MODE_EAX, nonce=nonce)
    staff11=Staffs.objects.all();
    staffss= Staffs.objects.values('id','admin','code','address','cmnd','profile_pic',
                                   'created_at','updated_at','updated_at','birthday','salary')
    l=[]
    for i in staffss:
        i['encrypt_key'] = encrypt(i['id'])
        i['id'] = i['id']
        i['salary'] = decrypt1(i['salary'])
        print(i['salary'])
        l.append(i)
    context = {
        "staffs": l,
...
def edit_staff(request, staff_id):
    ...
    form.fields['cmnd'].initial = staff.cmnd
    form.fields['profile_pic'].initial = staff.profile_pic
    form.fields['birthday'].initial = staff.birthday
    context = {
        "id": staff_id,
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                student_model.cmnd = cmnd

                if profile_pic_url is not None:
                    student_model.profile_pic = profile_pic_url
                    student_model.birthday = birthday
                student_model.save()
...
def add_student_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            gender = form.cleaned_data['gender']
            nienkhoa = form.cleaned_data['nienkhoa']
            birthday = form.cleaned_data['birthday']
            # Getting Profile Pic first
            # First Check whether the file is selected or not
            # Upload only if file is selected
            ...
            try:
                ...
                user.students.code = code
                user.students.address = address
                user.students.gender = gender
                user.students.cmnd = cmnd
                user.students.nienkhoa = nienkhoa
...
def edit_student(request, student_id):
    ...
    form.fields['cmnd'].initial = student.cmnd
    form.fields['birthday'].initial = student.birthday
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request, "Cập nhật sinh viên thất bại.")
                return redirect('/edit_student/' + student_id)
        else:
            return redirect('/edit_student/' + student_id)
...
def add_saff_gv_save(request):
    if request.method != "POST":
        ...
        form = AddTeacher_GVForm(request.POST, request.FILES)
        if form.is_valid():
            code = form.cleaned_data['code']
            first_name = form.cleaned_data['first_name']
            last_name = form.cleaned_data['last_name']
            ...
            birthday = form.cleaned_data['birthday']
            # Lấy Pic Hồ sơ trước
            # Đầu tiên Kiểm tra xem tệp đã được chọn hay chưa
            # Chỉ tải lên nếu tệp được chọn
...
def edit_staff_gv(request, staff_gv_id):
    ...
    form.fields['cmnd'].initial = staff_gv.cmnd
    form.fields['birthday'].initial = staff_gv.birthday
...
def edit_staff_gv_save(request):
    if request.method != "POST":
        ...
        if staff_gv_id is None:
            return redirect('/manage_staff_gv')

        form = EditTeacher_GVForm(request.POST, request.FILES)
        if form.is_valid():
            username = form.cleaned_data['username']
            first_name = form.cleaned_data['first_name']
            last_name = form.cleaned_data['last_name']
            ...
            try:
                ...
                staff_gv_hp_model.cmnd = cmnd

                if profile_pic_url is not None:
                    staff_gv_hp_model.profile_pic = profile_pic_url
                staff_gv_hp_model.birthday = birthday
...
def check_code_exist_change_password(request):
    code = request.POST.get("code")
    password = request.POST.get("password")
    if code != None and code != "":
        staff_obj = Staffs.objects.filter(code=code).exists()
    if staff_obj:
        staff1=Staffs.objects.get(code=code)
        customuser = CustomUser.objects.get(id=staff1.admin.id)
        ...
        messages.error(request, "Đổi Mật Khẩu Thất Bại.")
        return redirect('login')
...
#feedback message of gv
def staff_feedback_gv_message(request):
    feedbacks = FeedBackStaffs_GV.objects.all()
    context = {
        "feedbacks": feedbacks
    }
    return render(request, 'hod_template/staff_gv_feedback.html', context)


@csrf_exempt
def staff_feedback_gv_message_reply(request):
    feedback_id = request.POST.get('id')
    feedback_reply = request.POST.get('reply')

    try:
        feedback = FeedBackStaffs_GV.objects.get(id=feedback_id)
        feedback.feedback_reply = feedback_reply
        feedback.save()
        return HttpResponse("True")

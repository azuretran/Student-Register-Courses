...
from .models import CustomUser, Staffs, Courses, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, \
    LeaveReportStaff, FeedBackStaffs, StudentResult, PHONGHOC, result_tkb_dk


def staff_home(request):
    user = CustomUser.objects.get(id=request.user.id)
    staff_model = Staffs.objects.get(admin=user)
    subject = Subjects.objects.filter(staff_id=staff_model.admin).count
    context = {
...
def manage_subject_teach(request):

    user = CustomUser.objects.get(id=request.user.id)
    staff_model=Staffs.objects.get(admin=user)
    subject=Subjects.objects.filter(staff_id=staff_model.admin)
    context={
        "subject":subject,

    }
    return  render(request,"staff_template/manage_subject_teach_template.html",context)
def staff_gv_timetable(request):
    user = CustomUser.objects.get(id=request.user.id)
    staff_model = Staffs.objects.get(admin=user)
    subject = Subjects.objects.filter(staff_id=staff_model.admin)
    ...
    return render(request, "staff_template/staff_gv_timetable.html",context)